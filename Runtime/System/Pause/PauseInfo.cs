...
{
    /// <summary> ポーズ機構の管理状態を取得時点の不変値として表す。 </summary>
    public readonly struct PauseInfo : IEquatable<PauseInfo>
    {
        /// <summary>
        ///     ポーズ状態と購読件数から情報を生成する。
        ///     生成は<see cref="PauseQuery"/>の責務であり、
        ///     利用側は<see cref="PauseManager.GetPauseInfo"/>で取得する。
        /// </summary>
        ...
        /// <summary>
        ///     <see cref="PauseManager.IPausable"/>としてポーズ通知を購読している対象の件数。
        ///     解除し忘れが積み上がっていないかの確認に使う。
        /// </summary>
        public int PausableSubscriberCount { get; }

        /// <summary> 管理状態が等しいか判定する。 </summary>
        ...
        /// <summary> 管理状態に基づくハッシュコードを返す。 </summary>
