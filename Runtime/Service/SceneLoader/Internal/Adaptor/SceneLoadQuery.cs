...
{
    /// <summary>
    ///     Scene RegistryとEntityを公開InfoまたはView用Dtoへ変換する。
    /// </summary>
    internal sealed class SceneLoadQuery
    {
        ...
        {
            // Queryの生存期間を読み取り対象のRegistryへ固定する。
            _registry = registry ?? throw new ArgumentNullException(nameof(registry));
        }

        /// <summary>
        ///     指定したSceneの公開スナップショットを取得する。
        /// </summary>
        ...
        {
            // Registryが追跡していないSceneは公開状態として返せない。
            ...
            // Entityと同じ取得時点のActive Scene情報から不変な公開値を生成する。
        ...
        /// <summary>
        ///     追跡中Sceneの公開スナップショット一覧を返す。
        /// </summary>
        /// <returns> Scene名のordinal昇順で並んだ変更不能な一覧。 </returns>
        internal IReadOnlyList<SceneLoadInfo> GetInfos()
        {
            // 呼び出しごとの結果順を固定し、Active Scene名も一覧全体で同じ時点の値を使う。
            List<SceneLoadEntity> entities = GetSortedEntities();
            string activeSceneName = _registry.ActiveSceneName;
            SceneLoadInfo[] sceneInfos = new SceneLoadInfo[entities.Count];

            // 可変なEntityを外部へ渡さず、各要素を不変な公開値へ変換する。
            for (int i = 0; i < entities.Count; i++) { sceneInfos[i] = CreateInfo(entities[i], activeSceneName); }

            return Array.AsReadOnly(sceneInfos);
        }

        /// <summary>
        ///     追跡中SceneのView用更新値一覧を返す。
        /// </summary>
        /// <returns> Scene名のordinal昇順で並んだ変更不能な一覧。 </returns>
        internal IReadOnlyList<SceneLoadDto> GetDtos()
        {
            // 表示更新の比較が安定するよう、順序とActive Sceneの取得時点を一覧全体で揃える。
            List<SceneLoadEntity> entities = GetSortedEntities();
            string activeSceneName = _registry.ActiveSceneName;
            SceneLoadDto[] sceneDtos = new SceneLoadDto[entities.Count];

            // 表示に必要な値だけをEntityから切り出し、内部状態への参照を残さない。
        ...
        /// <summary>
        ///     2つのScene名が同じActive Sceneを示すか判定する。
        /// </summary>
        ...
        /// <summary>
        ///     RegistryのEntityをScene名のordinal昇順で複製する。
        /// </summary>
        /// <returns> 並べ替え済みEntity一覧。 </returns>
        private List<SceneLoadEntity> GetSortedEntities()
        {
            // Registryの列挙順を公開契約に漏らさないよう、複製してから名前順へ揃える。
            List<SceneLoadEntity> entities = new(_registry.Entities.Values);
            entities.Sort(CompareEntities);
            return entities;
        }

        /// <summary>
        ///     2つのEntityをScene名のordinal順で比較する。
        /// </summary>
