...
`Scene Init` はUnity 6000.3以降で使用できます。メインツールバー右側の音符アイコン付き`Symphony Framework`プルダウンから選びます。変更は`Assets/Resources/SymphonyFrameWork/SceneLoadConfig.asset`へ保存され、Inspectorの「エディタでの再生時にシーンをリセットしてロードを実行するか」と同じ設定へ反映されます。Play Mode開始後の変更は実行中のシーンを切り替えず、次のPlay Mode開始から反映されます。
