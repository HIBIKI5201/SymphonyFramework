...
### Breaking
- `SymphonyFrameWork.System.API` 名前空間を削除。Facadeクラスは2.1.0以前と同じ名前空間へ戻った。クラス名・メンバー・シグネチャ・シリアライズ形式はいずれも変更していないため、必要な対応は `using` の張り替えのみ。**移行方法:** `using SymphonyFrameWork.System.API;` を、使用しているFacadeに応じて次へ置き換える。
